...
Spray::Spray(DynamicMesh* mesh, Wand* wand)
{
...
void Spray::renderIntersection(MatrixStack* MVstack, GLint locationMV)
{
...
void Spray::render(MatrixStack* MVstack, GLint locationMV)
{
...
void Spray::firstSelect(DynamicMesh* mesh, Wand* wand)
{
...
void Spray::moveVertices(DynamicMesh* mesh, Wand* wand, float dT)
{
	float wPoint[4]; float newWPoint[4]; float Dirr[4]; float newDirr[4];
	float eVec1[3]; float eVec2[3];
	...
	if (success)
	{
		...
		for (int i = 0; i < tempSize; i++) {
			...
			if (pLength > 0.0f && oLength < radius) {
				...
				vPoint1[0] += vNorm2[0] * u*0.0002;
				vPoint1[1] += vNorm2[1] * u*0.0002;
				vPoint1[2] += vNorm2[2] * u*0.0002;
			...
			do {
				...
				if (mVInfoArray[index].selected == 0.0f){
					...
					if (pLength > 0.0f && oLength < radius) {
						...
						selectedVertices[selectedSize] = index; selectedSize++;
						mVInfoArray[index].selected = 2 + u;
					}
				}
